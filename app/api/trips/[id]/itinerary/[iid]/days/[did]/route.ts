...
import { updateDay, clearDay } from '@/app/lib/services/itinerary-service';
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string; iid: string; did: string }> }) {
  const ctx = await getUserContext(); if (!ctx) return NextResponse.json({ error: 'Not signed in' }, { status: 401 });
  const { id, did } = await params;
  await clearDay(ctx, Number(id), Number(did));
  return NextResponse.json({ ok: true });
}