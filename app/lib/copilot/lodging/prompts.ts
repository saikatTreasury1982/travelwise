...
export function lodgingSystemPrompt(opts: {
  ...
  routeHint: string;
  budgetHint?: string;
  destinationsHint?: string;   // e.g. "Tokyo, Kyoto, Osaka"
}): string {
...
1. WHICH destination this stay is for — if the trip has more than one destination, ask and offer them by name; if only one, use it without asking.${opts.destinationsHint ? ` The trip destinations are: ${opts.destinationsHint}.` : ''}
...
- On every suggest_stay call, set \`city\` to EXACTLY one of the trip destination cities${opts.destinationsHint ? ` (${opts.destinationsHint})` : ''} — this maps the stay to the right destination. Put the finer neighbourhood in \`area\` (e.g. area "Shinjuku, Tokyo", city "Tokyo").
