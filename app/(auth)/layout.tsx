// app/(auth)/layout.tsx
// Shared auth shell (Direction D). Left: dark panel with a budget preview.
// Right: the form (children). On narrow screens the panel collapses to a
// slim header (device-parity per amended guideline 1.1). Themeable: glass
// themes restyle .surface automatically.
import Link from 'next/link';

function Logo({ tone = 'panel' }: { tone?: 'panel' | 'ink' }) {
  const color = tone === 'panel' ? 'var(--panel-ink)' : 'var(--ink)';
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <span style={{ fontFamily: 'var(--font-display)', fontSize: 21, color }}>Travelwise</span>
    </div>
  );
}
...
export default function AuthLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row">
      {/* Left panel — dark, budget preview. Slim header on mobile. */}
      <aside
        className="relative overflow-hidden md:w-[44%] md:min-h-screen"
        style={{
          backgroundColor: 'var(--panel)',
          backgroundImage:
            'radial-gradient(620px 460px at 26% 18%, rgba(52,96,156,0.46), transparent 64%), radial-gradient(520px 420px at 84% 76%, rgba(232,163,61,0.26), transparent 62%)',
        }}
      >
        <div className="flex flex-col justify-between h-full box-border p-6 md:p-12 gap-8">
          ...
          <div className="hidden md:flex flex-col gap-7">
            ...
            {/* Budget preview card — the product thesis on the first screen */}
